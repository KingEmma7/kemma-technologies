...
export const metadata = pageMetadata({ title: "Website pricing", description: `${starterEstimate.launchApplied ? "Launch" : "Standard"} website package from ${money(starterEstimate.packageMin)}: up to ${catalogue.starter.pages_max} short pages, eligible domain and basic hosting for one year.${starterEstimate.launchApplied ? ` ${launchTerms}` : " Renewals disclosed before deposit."}`, path: "/pricing" });
const exampleNames: Record<string, [string, string]> = {
  starter: ["Business website", `Up to ${catalogue.starter.pages_max} short pages, contact links and one enquiry form.`],
  catalogue: ["Catalogue & WhatsApp ordering", "Up to 10 products and an order summary. You confirm availability and delivery."],
  links: ["Catalogue with payment links", "Supported payment links, with manual payment checks and order matching."],
  faq: ["Catalogue, payment links & fixed FAQs", "Add up to 15 approved answers and a human handoff. No generative AI."],
  store: ["Standard store & customer accounts", "Cart, checkout, a gateway, order administration and standard customer logins."],
};
const questions = [
  ...(catalogue.launch.enabled ? [["How does the launch price work?", `${launchTerms} The launch price is ${money(catalogue.baseFee + catalogue.packageFee - catalogue.launch.discount)} against a standard package price of ${money(catalogue.baseFee + catalogue.packageFee)}. Both include the same five-page scope and first-year eligible standard domain and basic hosting. The ${money(catalogue.launch.discount)} discount applies once to the bundled project, not to each addition, supplier charge or care plan. It does not stack with build-only. Enquiring does not reserve a place; Kemma confirms availability and the full quote before your deposit.`]] : []),
  ...
  ["Does the starter include everything in a demo?", "No. The demos show possible journeys and distinct visual directions. The starter adapts a prepared layout within the stated content limits. Bespoke composition, extra pages, live inventory, accounts, payment operations and booking capacity are scoped separately. Demo forms are simulations."],
...
export default function PricingPage() {
  ...
  return <>
    <PageIntro label="Website pricing" title="Start with what your business needs." description="A useful website, a clear starting price, and room to grow. First-year essentials included; future costs explained before you commit." />
    <section className={`wrap ${styles.starter}`} aria-labelledby="starter-title">
      <div className={styles.startPrice}><h2 id="starter-title">Business website</h2>
        <p>{starterEstimate.launchApplied ? "Launch price from" : "Standard price from"}<strong>{money(starterEstimate.packageMin)}</strong></p>
        {starterEstimate.launchApplied && <p className={styles.standardPrice}>Standard price <s>{money(starterEstimate.standardPackageMin)}</s></p>}
        <span>Up to {base.pages_max} short pages · first-year domain & basic hosting included</span>
        {starterEstimate.launchApplied && <p className={styles.hint}>{launchTerms}</p>}
        <a className="solid-button" href="#estimate">Explore your estimate</a>
        <p className={styles.hint}>Eligible standard domain and suitable basic hosting for one year. Premium domains, specialist infrastructure, subscriptions and optional care cost extra. Year-two renewal details are agreed before deposit.</p>
        <p className={styles.hint}>Already have suitable domain and hosting services? Build-only starts at {money(catalogue.baseFee)} with the same build scope; no launch discount applies. No payment is taken here.</p>
      </div>
      <div className={styles.starterScope}><h3>A useful starting point, with the essentials included.</h3>
        <ul>
          <li>Up to {base.pages_max} short pages, about {base.supplied_words_max.toLocaleString()} supplied words and {base.supplied_images_max} supplied images.</li>
        ...
        <p className={styles.hint}>Writing, logos, photography, paid assets, ongoing edits, content editing tools, catalogues, accounts, payments and bespoke 3D are additional scope. Search rankings are not guaranteed.</p>
      </div>
    </section>
    <section className={`wrap ${styles.section}`} aria-labelledby="example-title">
      <div className={styles.sectionHeading}><h2 id="example-title">A few ways to start.</h2><p>{starterEstimate.launchApplied ? "Launch package starting prices, including one discount per eligible project." : "Standard package starting prices."} Choose the functions you need. Specialist infrastructure and additional services remain subject to a quote with credit for bundled work.</p></div>
      <div className={styles.examples}>
        {Object.entries(exampleNames).map(([id, [name, description]]) => <Link key={id} href={`/pricing?path=${id}#estimate`} className={styles.example}>
          <h3>{name}</h3><p>{description}</p><strong>From {money(estimate(initialSelection(id)).packageMin)}</strong><span>Explore this path</span>
        </Link>)}
      </div>
      <p className={styles.hint}>The store floor assumes a compatible reusable platform, before any required infrastructure uplift. Bespoke commerce, a custom frontend and specialist operations need a separate quote. Included domain and hosting work is credited against any replacement setup.</p>
    </section>
    <section className={`wrap ${styles.section}`} id="estimate" aria-labelledby="estimate-title">
      <div className={styles.sectionHeading}><h2 id="estimate-title">Build a starting estimate.</h2><p>Choose one selling path, then add the work that matters. Included features are counted once. Unknown required costs remain visible.</p></div>
    ...
    <section className={`wrap ${styles.section}`} id="additions" aria-labelledby="additions-title">
    ...
    <section className={`wrap ${styles.section}`} id="running-costs" aria-labelledby="running-title">
    ...
    <section className={`wrap ${styles.section}`} aria-labelledby="process-title">
    ...
    <section className={`wrap ${styles.section}`} aria-labelledby="questions-title">
    ...
    <section className={`wrap ${styles.finalCta}`}><h2>Start with your business.</h2><p>Try a demo, put together an estimate, or tell us what you need. We’ll turn that into a clear written scope.</p><div><Link className="text-link" href="/demos">Explore the demos</Link><Link className="text-link" href="/contact">Talk through your project</Link></div></section>
  </>;
}
