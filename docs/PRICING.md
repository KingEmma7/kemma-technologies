...
The owner approved this launch package for production on 14 September 2026 and removed fixed page, product, word and image caps on 21 September 2026. The catalogue at `lib/pricing/catalogue.json` is the maintained website source; provenance is in `docs/pricing-source-provenance.json`. No supplier signup, payment or message is performed by this page.
...
- Both builds have no fixed page, product, supplied-word or supplied-image cap. The written scope agrees the page structure, supplied content, content preparation and custom functionality before work starts. It includes one consolidated revision, basic SEO, one five-field enquiry form, WhatsApp links, handover and 30 days of agreed defect correction. This removes arbitrary capacity limits without promising unlimited custom design, writing, product-data entry or other labour.
- All 38 addon IDs, prices, dependencies and overlap rules are retained. The legacy `page` and `product_entry` IDs now describe optional Kemma content-preparation work rather than capacity charges, preserving saved selections and arithmetic. The store includes catalogue and gateway work; platform-provided configuration can receive explicit credits. Combined 3D/scroll includes its viewer and scroll work but still needs a usable model or a model quote. Unit quantities, prerequisites and custom-scope quote flags remain enforced.
